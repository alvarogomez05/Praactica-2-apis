...
const escogerDestino = (event) => {
    if (event.target.tagName === "IMG") {
        createObject(event.target);
    }
}

const createObject = (data) => {
    // console.log(data.nextElementSibling.firstElementChild.textContent)
    // console.log(data.nextElementSibling.lastElementChild.textContent)
    // console.log(data.src)


    let newObject = new Billete(data.nextElementSibling.firstElementChild.textContent,
        data.nextElementSibling.lastElementChild.textContent,
        data.src
    );

    // console.log(newObject)
    guardarLocalStorage(newObject);
}

const guardarLocalStorage = (object) =>{
    // console.log(object)
    let aux = JSON.parse(localStorage.getItem("billetes"));

    if(aux === null){
      aux = [];
    }
  
    //si no está vacío simplemente añadimos el producto al array de productos
    //añadir al array
    aux.push(object);
    console.log(aux)
...
cards.addEventListener("click", escogerDestino)